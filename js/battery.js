...
//robohash api status url
const roboHashStatus = document.querySelector('#roboHashStatus');

/* Functions
-------------------------------------------------- */
//set the seed of the robot image generator based on the value of the users battery

//robohash api connection function
...
async function fetchRoboImage(seed) {
    const roboHashbaseURL = "https://robohash.org/";
    const response = await fetch(`${roboHashbaseURL}${seed}`);


    if (!response.ok) {
        roboHashStatus.textContent = "Error, cannot generate robot image."
    }


    const imageURL = response.url;
    document.getElementById("roboImage").src = imageURL;
}
